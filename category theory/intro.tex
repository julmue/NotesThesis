...
Ein Morphismus $A \rightarrow{f} B$ wird Isomorphismus (oder umkehrbare Abbildung)
genannt, wenn es eine Abbildung $B \rightarrow{g} A$ gibt für welche die 
...
wenn es mindestens einen Isomorphismus $A \rightarrow{B} B$ gibt.

\begin{remark}{Gleichmächtigkeit ist eine Äquivalenzrelation}
Die Gleichmächtigkeits zwischen zwei Mengen ist eine Äquivalenzrelation:
\begin{enumerate}
...
und $g^{-1}: C → B$ Morphismen mit den Eigenschaften: 
\begin{enumerate*} 
\item $f ∘ f^{-1} = 1_A$ und
\item $f^{-1} ∘ f = 1_B$ 
\item $g ∘ g^{-1} = 1_B$ und
\item $g^{-1} ∘ g = 1_C$ 
\end{enumerate}. Dann gilt:

\begin{subproof}[\ref{def:IsoRef}]
Aufgrund der Identitätsabbildung $1_A$.
\end{subproof}
\begin{subproof}[\ref{def:IsoSym}] 
Aufgund der Voraussetzung.
\end{subproof}
\begin{subproof}[\ref{def:IsoTra}]
\begin{align*}
